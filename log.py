...
def set_up_logger(args, sys_argv):
    ...
    runtime_id = '{}-{}-{}'.format(str(time.time()), args.data, args.pos_dim)
    ...
    get_checkpoint_path = checkpoint_dir + 'best_checkpoint.pth'
...
def save_oneline_result(dir, args, test_results):
    ...
    with open(dir+'oneline_results.txt', 'a') as f:
        elements = [str(e) for e in [args.data, args.mode[0], n_layer, 'k'.join(n_degree), args.pos_dim,
                                     *[str(v)[:6] for v in test_results]]]
