...
class ResidualBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.norm_end1 = RMSNorm(config.d_model)
        self.norm_end2 = RMSNorm(config.d_model)
    ...
    def forward(self, x):
        ...
        x = self.norm_end1(x)  #添加一层norm @lys

        #添加FFN 已经另外一个残差连接 @lys
        output = self.norm_end2(self.ffn(x) + x)
